...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double applied_discount = apply_discount(daily_rate(hourly_rate) * 22.0, discount);
    double result = std::ceil(applied_discount);
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double days = budget / apply_discount(daily_rate(hourly_rate), discount);
